...
# class Lid2d_Dataset_Time_Dependent(Dataset):
#     def __init__(self, 
#                  data_path, 
#                  mode="test",
#                  all_length = 41,
...
#         super(Lid2d_Dataset_Time_Dependent, self).__init__()
#         assert mode in ["train", "test"]
        
#         self.all_length = all_length # <= 41
...
#         if self.mode == "train":
#             with open("./data_split/train_list_stationary_all.txt", "r") as f:
#                 for item in f.readlines():
#                     self.dataloc.append(item.strip())
...
#             # print(f"{self.mode}, open {idx}")
            
#             if self.window_length == (self.all_length//self.delta_t - self.input_step):
#                 t = 0
#             else:
#                 t = random.randint(0, self.all_length//self.delta_t - self.window_length - self.input_step)
            
#             Length = t + (self.window_length + self.input_step) * self.delta_t
...
#             Re = data_['conditions']['Re'][()]
#             v_wall = data_['conditions']['v_wall'][()]
#             v_wall = round(v_wall, 2)
...
#             condition = torch.tensor([Re, v_wall]).unsqueeze(0).repeat(pressure.shape[0], 1).float()
            

#             # print(f"{t}->{self.delta_t}->{Length}")
            
            
#             key_1 = '{}'.format(idx)
#             # ['cells', 'node_type', 'pos', 'pressure', 'velocity']
            
#             cells = data_[key_1]['cells'][0]
#             node_type = data_[key_1]['node_type'][0]
#             pos = data_[key_1]['pos'][0]
            
#             pressure = data_[key_1]['pressure'][t:Length:self.delta_t]
#             velocity = data_[key_1]['velocity'][t:Length:self.delta_t]
            
#             all_time = np.arange(t, Length, delta_t)
#             t = (all_time + 1) * self.dt
            
#             return cells, node_type, pos, pressure, velocity, t

#         self.data_files = partial(open_h5py_file, self.data_dir)
...
#         cells, node_type, pos, pressure, velocity, t = self.data_files(idx)
#         clusters = self.data_clusters(idx)
...
if __name__ == "__main__":
    dataset = Lid2d_Dataset(data_path = "../lid2d_stationary/", 
            mode="test",
            normalize = True,)
    for data in dataset:
