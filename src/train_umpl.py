...
from .all_loss_umpl import get_val_loss_lid2d
...
class NLL_Loss_mask(nn.Module):
    ...
    def forward(self, y_pred, log_var, y_true, mask):
        # 将归一化的不确定度映射为有效的方差值
        return torch.mean(mask*(0.5 * torch.exp(-log_var) * (y_true - y_pred)**2 + 0.5 * log_var))
...
def get_coe(args, epoch):
    return 0.5 + epoch/args.train["epoch"]*0.5
...
def smooth_random_field(node_pos, edges, r, eps=1e-6):
    ...
    edges_flat = edges_offset.reshape(B * E, 2).long()  # (B*E, 2)
    ...
    weight = 1.0 / (dist + eps).to(device)
    contrib = weight * r_flat[j].to(device)

    # Step 3: scatter to accumulate
    xi_accum = torch.zeros_like(r_flat).to(device)
    xi_accum.index_add_(0, i, contrib)

    # Step 4: reshape back to (B, N)
    xi = torch.sign(xi_accum).view(B, N)
    return xi.unsqueeze(-1).to(device)
...
def train(args, model_t, model_s, train_dataloader, test_dataloader, optim_t, optim_s, epoch, local_rank):
    ...
    for i in range(len(train_dataloader)):
        ...
        node_pos = input_test['node_pos'].to(local_rank)
        edges = input_test['edges'].to(local_rank)
        ...
        with torch.no_grad():
            ...
            pseudo_state = pred_state + ema_std*smooth_random_field(node_pos, edges, torch.randn_like(node_pos))
        ...
        loss_l2_test = NLL_Loss_mask()(pred_test, pred_test_unc, pseudo_state, pseudo_mask)
        ...
        loss_s_nll = NLL_Loss_mask()(pred_state_train, pred_state_unc, train_gt, sup_mask)
        loss_s = loss_s_nll
        loss_s.backward()
        optim_s.step()
